  ...
  return (
    <div className="mx-auto flex max-w-sm flex-col gap-2" style={{ height: 'calc(100svh - 69px)' }}>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/5">
        ...
        <div className="no-scrollbar flex flex-1 flex-col gap-1.5 overflow-y-auto p-1.5">
          {PL_CLUBS.map((club) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'flex w-full flex-none items-center gap-3 overflow-hidden rounded-xl px-4 py-3 text-left transition-all',
                  isSelected
                    ? 'ring-2 ring-[#00FF85] ring-offset-1 ring-offset-[#38003C]'
                    : 'ring-1 ring-white/10 hover:ring-white/20',
                )}
                style={{
                  background: `linear-gradient(135deg, ${club.primary}cc 0%, ${club.secondary}44 100%)`,
                }}
              >
                <span className="w-9 flex-none text-[10px] font-black uppercase tracking-widest text-white/50">{club.short}</span>
                <span className="flex-1 text-sm font-black text-white">{club.name}</span>
                {isSelected && <span className="h-2 w-2 flex-none rounded-full bg-[#00FF85]" />}
              </button>
            );
          })}
        </div>
